...
bool _dictionaryScript(String value) =>
    ...
    RegExp(r'[぀-ヿ㐀-鿿฀-๿຀-໿ក-៿က-႟]').hasMatch(value);

/// Chains whose morphing word holds an astral or combining character: the
/// oracle splits by code unit, the port by grapheme cluster (DEV-007).
const Set<String> graphemeMorphChainLabels = {
  'extra:emoji',
  'extra-nonum:emoji',
  'extra:combining',
  'extra-nonum:combining',
};
...
void main() {
  group('segmentText (oracle:segment-text)', () {
    ...
    for (final c in cases) {
      ...
      if (gap) skipped++;
      test(
        '${show(value)} numbers=$numbers',
        () {
          final norm = MintNormalizer();
          final segs = segmentText(
            value,
            c['locale'] as String,
            numbers: numbers,
          );
        ...
        skip: gap
            ? 'ICU dictionary segmentation (SEGMENTER_KNOWN_GAPS.md)'
            : false,
      );
    }
  ...
  group('diff chains (oracle:chains)', () {
    ...
    for (final chain in chains) {
      ...
      final gap = steps.any(
        (s) =>
            _dictionaryScript(s['value'] as String) &&
            (s['value'] as String).contains(' '),
      );
      final graphemeGap = graphemeMorphChainLabels.contains(label);
      test(
        label,
        () {
          final norm = MintNormalizer();
          ...
          for (final step in steps) {
            ...
            final actualSplits = splits.map(
              (k, v) => MapEntry(norm(k), norm.segments(v)),
            );
            expect(
              actualSplits,
              expectedSplits,
              reason: 'splits of step ${show(value)}',
            );
            prev = segs;
          }
        },
      ...
      for (final chain in chains) {
        ...
        for (final step in chain['steps'] as List) {
          ...
          final segs = prev.isEmpty
              ? segmentText(
                  value,
                  chain['locale'] as String,
                  numbers: chain['numbers'] as bool,
                )
              : diffSegments(
                  prev,
                  value,
                  ...
                  DiffOptions(
                    numbers: chain['numbers'] as bool,
                    cursorIndex: step['cursorIndex'] as int?,
                  ),
                ).segments;
          ...
          expect(
            ids.toSet().length,
            ids.length,
            reason: 'duplicate ID in ${show(value)}: $ids',
          );
          final rendered = segs
              .map((s) => s.string)
              .join()
          ...
          expect(
            ...
            reason: 'rendered text of ${show(value)}',
          );
          for (final s in segs) {
  ...
  group('diff bounds', () {
    test(
      'bails to segmentText past MAX_LCS_CELLS and still renders the text',
      () {
        ...
        expect(
          r.segments.map((s) => s.string).join().replaceAll(nbsp, ' '),
          newText,
        );
    ...
    test('skips similarity pairing past MAX_MORPH_PAIRINGS', () {
      ...
      final r = diffSegments(
        ...
        newText,
        'en',
        const DiffOptions(numbers: false),
      );
      expect(r.splits, isEmpty);
      expect(
        r.segments.map((s) => s.string).join().replaceAll(nbsp, ' '),
