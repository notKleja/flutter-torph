...
const Map<String, String> excludedCorpusIds = {
  'RTLC-048':
      'LRM (U+200E) is a zero-width bidi control; no visual box to compare',
  'RTLC-049':
      'RLM (U+200F) is a zero-width bidi control; no visual box to compare',
...
List<ItemFrame> liveNonBreak(TextMorphSnapshot snapshot) =>
    snapshot.liveItems.where((i) => !i.isBreak).toList();
...
void main() {
  ...
  test(
    'corpus loaded from oracle/fixtures/rtl/corpus.json (not the fallback)',
    () {
      expect(
        rtlCorpusIsFallback,
        isFalse,
        reason:
            'oracle/fixtures/rtl/corpus.json must exist and be non-empty for this audit',
      );
  ...
  for (final c in cases) {
    ...
    testWidgets(
      '${c.id} (${c.category}): "${c.text.replaceAll('\n', '\\n')}"',
      (tester) async {
        final direction = c.direction == 'ltr'
            ? TextDirection.ltr
            : TextDirection.rtl;
        ...
        expect(
          ...
          textLines.length,
          reason:
              'Torph produced ${torphLines.length} line(s) for '
        ...
        for (var i = 0; i < textLines.length; i++) {
          ...
          expect(
            ...
            plainVisual,
            reason:
                '${c.id} line $i: Torph visual order must match the '
  ...
  group('morph safety under bidi', () {
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(
          host(TextMorph(value: 999), direction: TextDirection.rtl),
        );
        await tester.pumpWidget(
          host(TextMorph(value: 1000), direction: TextDirection.rtl),
        );
        ...
        final settledLive = liveNonBreak(settled)
          ..sort((a, b) => a.visualRect.left.compareTo(b.visualRect.left));
        expect(
          [for (final i in settledLive) i.text],
          ['1', ',', '0', '0', '0'],
        );
        expect(
          [for (final i in settledLive) i.visualRect.left],
          [0, 20, 40, 60, 80],
        );
        expect(
          midOrder,
          ['1', ',', '0', '0', '0'],
          reason:
              'order must already be settled-correct mid-morph, not just at rest',
        );
      },
    );
      ...
      (tester) async {
        Future<void> expectSides(TextMorphSnapshot snapshot) async {
          expect(
            ...
            reason:
                'ريال must stay at the visual left, السعر at the visual right',
          );
        }
        ...
        expect(
          liveNonBreak(
            settled,
          ).firstWhere((i) => i.kind == SegmentKind.digit && i.text == '5'),
          isNotNull,
