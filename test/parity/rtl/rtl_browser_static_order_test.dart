...
const Set<String> _zeroWidthSpace = {'RTLC-048', 'RTLC-049', 'RTLC-050', 'RTLC-051', 'RTLC-052', 'RTLC-054'};
...
void main() {
  ...
  final files = dir.listSync().whereType<File>().where((f) => RegExp(r'RTLC-\d+\.json$').hasMatch(f.path)).toList()
  ...
  for (final file in files) {
    ...
    final direction = b['direction'] == 'rtl' ? TextDirection.rtl : TextDirection.ltr;
    final rest = b['torphAtRest'] as Map<String, dynamic>;

    testWidgets('$id ${jsonEncode(text)}', (tester) async {
      await tester.pumpWidget(host(
          TextMorph(value: text, locale: Locale(b['locale'] as String), bidi: false),
          direction: direction));
      await tester.pump(const Duration(seconds: 1));
      final snap = snapshotOf(tester);

      final browserItems = (rest['items'] as List).cast<Map<String, dynamic>>().where((i) => i['tag'] != 'br').toList();
      ...
      bool keep(String t) => !skip || t.replaceAll(RegExp(r'[\u200e\u200f\u2066-\u2069]'), '').trim().isNotEmpty;

      final browserLines = <int, List<Map<String, dynamic>>>{};
      for (final i in browserItems) {
        browserLines.putIfAbsent(((i['rect'] as Map)['y'] as num).round(), () => []).add(i);
      }
      ...
      final browserOrder = [
        for (final key in browserLines.keys.toList()..sort())
          (browserLines[key]!..sort((a, c) => ((a['rect'] as Map)['x'] as num).compareTo((c['rect'] as Map)['x'] as num)))
      ...
      final flutterOrder = [
        for (final key in flutterLines.keys.toList()..sort())
          (flutterLines[key]!..sort((a, c) => a.visualRect.left.compareTo(c.visualRect.left)))
              .map((i) => _norm(i.text))
              .where(keep)
              .toList(),
      ];
      expect(flutterOrder, browserOrder, reason: 'visual left-to-right order per line');
    });
  }
}
