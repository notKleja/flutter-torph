...
void parseUcd(
  String path,
  void Function(int start, int end, String value) emit,
) {
...
void main() {
  ...
  constant(
    'catExtend',
    catExtend,
    r'ICU $Extend (Word_Break=Extend minus Han).',
  );
  ...
  void writeList(String name, List<int> list, String doc) {
    ...
    for (var i = 0; i < list.length; i += 16) {
      final chunk = list.sublist(
        i,
        i + 16 > list.length ? list.length : i + 16,
      );
  ...
  stdout.writeln(
    'wrote lib/src/core/word_break_table.dart '
