...
MorphEngine engine({FakeMeasurer? measurer, MorphConfig? config}) =>
    MorphEngine(measurer: measurer ?? FakeMeasurer(), config: config ?? MorphConfig());
...
void main() {
  ...
  test('hello world → hello: world exits pinned where it was and fades over 25%', () {
    final e = engine();
    e.update('hello world', disabled: false);
    e.frame(0);
    e.update('hello', disabled: false);
  ...
  test('callbacks: start before each morph, exactly one of complete/cancel', () {
    final log = <String>[];
    final e = engine(
        config: MorphConfig(
