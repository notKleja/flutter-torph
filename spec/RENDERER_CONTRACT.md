...
4. Horizontal: `lineWidth = Σ width_i`; `containerWidth = width ?? naturalWidth` where `naturalWidth = max lineWidth`; `free = containerWidth − lineWidth`. If `free ≤ 0` the line overflows and is start-aligned (LTR: left = 0; RTL: left = containerWidth − lineWidth). Else `left = free × {left: 0, center: 0.5, right: 1}`. LTR: `x_i = left + Σ_{j<i} width_j`. RTL: `x_i = left + lineWidth − Σ_{j≤i} width_j` (items flow from the right, logical order preserved — no bidi reordering). With `bidi: true` (the default, DEV-006) both directions instead take `x_i` from `getBoxesForSelection` over a single line painter, and `lineWidth` from that painter.
